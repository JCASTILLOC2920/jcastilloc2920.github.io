...
// Initialization Logic for Antibody Table
document.addEventListener('DOMContentLoaded', function () {
    const modal = document.getElementById('antibodyModal');
    const span = document.querySelector('.ab-close');
    ...
    const cells = document.querySelectorAll('.antibody-table td');
    cells.forEach(cell => {
        const text = cell.innerText.trim();
        if (ANTIBODY_DATA[text]) {
            cell.classList.add('antibody-item');
            cell.title = "Haz clic para ver su uso clínico";

            cell.addEventListener('click', function () {
                title.innerText = text;
                info.innerText = ANTIBODY_DATA[text];
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function (e) {
            ...
            cells.forEach(cell => {
                const markerName = cell.innerText.toLowerCase();
